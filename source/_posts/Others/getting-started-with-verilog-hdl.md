...
类似于 C 语言函数的效果，建立了一个模块，就可以被任意多个其他模块调用。

定义模块并不能算使用，只有调用模块才能算作使用。

注意，这里指的`被其他模块调用`，是其他模块中生成了一个该模块的复制（然后执行该部分），而不完全是函数的概念，没有函数的`退出调用`、`函数栈`、`递归`等的概念。

#### 函数和任务调用

上面已经说的差不多了，函数调用和模块调用不一样。后续章节会详细介绍函数和任务的声明和调用方法。
...
1. 十进制计数法，如 `2.0`，`5.678`，`11572.12`，`0.1`
2. 科学计数法

描述|值
`23_5.1e2`|`23510.0`；忽略下划线
`3.6E2`|`360.0e`（与`E`相同）
`5E-4`|`0.0005`

Verilog语言定义了实数如何隐式地转换为整数：实数通过**四舍五入**被转换为最相近的整数。

#### 字符串

> 字符串是双引号内的字符序列，用一串 8 位二进制 ASCII 码的形式表示，每一个 8 位二进制 ASCII 码代表一个字符。  
> 例如：字符串 `ab` 等价于 `16'h5758`。  
> 如果字符串被用作 Verilog 表达式或复制语句的操作数，则字符串被看做无符号整数序列。

示例：字符串变量的声明

存储 12 个字符的字符串 `Hello China！` 需要 8*12（即96位）宽的寄存器。
...
> 可以使用 Verilog HDL 的操作符对字符串进行处理，被操作符处理的数据是 8 位 ASCII 码的序列。  
> 在操作过程中，如果声明的字符串变量位数大于字符串实际长度，则在赋值操作后，字符串变量的左端（即高位）补 0。
...
begin
    ...
    $display("%s is stored as %h",stringvar,stringvar); //$display类似 C 语言的 printf
    stringvar={stringvar."!!!"};
...
Verilog 分为三大流派：行为级建模、结构级建模，以及行为级和结构级的混合建模。
...
`#t` 的含义应为：等待 `t` 时间以后执行（而不是在 `t` 时刻执行）。  
也就是说，如果有两个 `#` 叠加，后者语句的执行时间应是二者时间的叠加。在[语句块](#语句块)里会有一个例子说明。
