...
教材讲的是 SQL Server 2000。博主使用的是 Microsoft SQL Server 2000 + Microsoft SQL Management Studio 18。

当然也可以使用最新版的 SQL Server。
...
CREATE VIEW DiagView
    AS
       SELECT DGno, P.Pno, Pname, Doc.Dno, 
...
```sql
-- 为消化内科诊断的患者信息建立一个视图，要求进行修改和插入操作时仍需保证该视图只有消化内科的患者
CREATE VIEW DiagView
        ...
        WHERE Diag.Dno = Doc.Dno
          ...
          AND Doc.DdeptNo IN 
              (SELECT DeptNo
                ...
                WHERE DeptName = '消化内科')
  WITH CHECK OPTION
-- 这意味着，以后对该视图进行修改、删除操作时，RDBMS 会自动加上 DeptName='消化内科' 的条件
-- 若进行插入操作， RDBMS 自动检查插入医生的 DeptName 属性列的值是否为消化内科，如果不是则拒绝执行操作
```
