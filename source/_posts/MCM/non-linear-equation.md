...
$Ax=b$，右端有一点扰动量 $\delta b$：

$$A(x+\delta x) = b + \delta b$$

$A\delta x = \delta b, \delta x = A^{-1}\delta b$

啊啊啊补不完了

$\\|A\\| \cdot \\|A\^{-1}\|$ 越大，即使 b 小，x 也会很大

因此，定义条件数：$\\|A\\| \cdot \\|A\^{-1}\|$ 越大，即使 b 小，x 也会很大

因此，定义条件数：

$$Cond(A) = \\|A\\| \cdot \\|A\^{-1}\|$$

条件数很大时，$Ax=b$ 是病态问题；条件数很小时（几百几千几万），$Ax=b$ 是良态问题。

15阶希尔伯特矩阵的条件数为 $10^{10^{16}}$。。。。。
