...
分治复杂度计算：  
> 若 $T(n) = k \cdot O(\frac{n}{2}) + \Theta(f(n))$，则 $T(n) = O(n^{log_2 k})\cdot T(1) + O(n*f(n))$。（构造等比数列或列出递归树证明）
...
问题 X 被多项式规约（polynomial reduces to）问题 Y，定义为对于问题 X 的任意一种情形，都能在多项式次标准操作加多项式次调用问题 Y 的方法/模型。  
记作：$X \leq _p Y$。
...
3. Reduction by encoding with gadgets.(利用一些**小技巧**进行规约)

P8-15    

#### SAT 问题

SAT 问题（Satisfiability）是给定一个析取范式（CNF），判定是否存在一种赋值，使得该范式成立。

3-SAT 即是，每个 Clause（每个析取的语句）的变量数不超过 3.
...
证明：显然 k 是图的独立集大小的上界。若该图的最大独立集大小为 k，则每个三角形必有一个顶点在该独立集中，且这些点不会同时出现 $x$ 和 $\overbar{x}$ 的情况（否则这两点会被相连，与独立集定义矛盾），则可使取的点（$x$ 或 $\overbar{y}$）的值为真，即是 3-SAT 的解。  
若 3-SAT 有解，则独立集可取解中所有值为真的点，以及值为假的点的取反（即若 $x$ 为假，取所有 $\overbar{x}$），对于同一三角形中的点，可在取到的集合去掉，即可得到一个大小为 k 的独立集。

#### 自身归约

自身归约是一种性质，搜索问题 $\leq _p$ 决定问题（于是对于这种问题，我们要想证明 NP，只需要证明决定是 NP 的）。

对于目前所有问题，都可以证明有自身归约的性质（但不代表所有问题一定都有自身归约的性质）

### NP

#### 决定性问题

X 是一个字符串（当然也可以是数字）的集合，s 是一个字符串，决定问题是：需要判断 s 是否在 X 中。
...
证明 NP：只需证明能在多项式时间内验证。

证明素数是NP？只能调用 AKS 跑一遍。
